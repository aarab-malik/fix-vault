...
from app.dependencies import CredentialsUser, CurrentUser, DbSession
...
from app.services.openai_service import extract_incident_from_credentials
...
async def draft_incident(body: IncidentDraftRequest, user: CredentialsUser):
    creds = get_user_credentials(user)
    return extract_incident_from_credentials(creds, body.notes)
...
async def create(body: IncidentCreateRequest, user: CredentialsUser, db: DbSession):
...
async def update(incident_id: str, body: IncidentUpdateRequest, user: CredentialsUser, db: DbSession):
...
async def remove(incident_id: str, user: CredentialsUser, db: DbSession):
    creds = get_user_credentials(user)
...
async def reindex(incident_id: str, user: CredentialsUser, db: DbSession):
    incident = await get_incident(db, user.id, incident_id)
    if not incident:
        raise HTTPException(status_code=404, detail="Incident not found")
    creds = get_user_credentials(user)
